...
<Update label="September 24, 2026" tags={["API", "Companies"]}>
## Companies have a city, state, postal code, country and email

- `POST /companies` and `PATCH /companies/{id}` accept `city`, `state`, `postal_code`, `country` and `email`, and every company response returns them.
- `country` is stored as a two-letter code (`GR`). Send the code or an English country name; an unknown country is refused with `422`.
- `email` must be a valid address. Send an empty string to clear any of these fields.
</Update>

<Update label="September 24, 2026" tags={["Developer Portal", "MCP"]}>
## See your request allowance in the Developer Portal

- **API keys** now shows the requests per minute your workspace's keys share. Only a key with its own custom limit shows a figure of its own.
- **Playground** responses show how much of this minute's allowance is left, and after a `429` how many seconds until you can retry.
- **MCP server:** the operation catalog now includes editing and deleting contact notes, the Facebook Page settings, picture and cover endpoints, and the Reel limit check.
</Update>

<Update label="September 24, 2026" tags={["API", "Improvement"]}>
## Request limits follow your connected accounts, one allowance per workspace

The [request limits](/guides/rate-limiting) now apply as published: 60, 600 or 1,200 requests per minute depending on the social accounts connected. Until now every key stayed at 60 requests per minute.

- **One allowance per workspace.** All API keys in a workspace, including the keys MCP connections create, draw on one per-minute allowance. Creating more keys no longer adds requests.
- **Test keys** (`sqk_test_`) have their own allowance of the same size, so sandbox traffic never uses up your live one.
- **A key with a custom rate limit** keeps an allowance of its own.
- **`X-RateLimit-Reset`** is always the start of the next minute.
</Update>

<Update label="September 22, 2026" tags={["API", "Contacts"]}>
## Contact notes can be edited, formatted, coloured and pinned

Notes gained a title, formatting, a colour, a pin and attachments, and can now be edited and deleted over the API.

- **New endpoints:** `PATCH /contacts/{id}/notes/{note_id}` edits a note (only the fields you send change) and `DELETE /contacts/{id}/notes/{note_id}` removes it.
- **New fields** on `POST /contacts/{id}/notes` and the edit: `title` (up to 120 characters), `body_html` (bold, italic, underline, strike, lists, quotes and http(s) / mailto / tel links - anything else is removed), `color` (`yellow`, `green`, `blue`, `gray`, `teal`, `purple`, `orange`, `pink`, `violet`, `cyan`), `pinned` and `attachment_file_ids` (up to 5 files already on the contact).
- **`body` stays plain text** on every response, so existing integrations keep working; `body_html` is the formatted version when there is one.
- **A note needs text, a title or an attachment**, so `body` is no longer strictly required.
- **Automations:** editing a note's title, text, colour or attachments starts the new "Note changed" trigger; pinning does not. `{{note.title}}` is a new merge tag.
- `GET /contacts/{id}/notes` now lists pinned notes first and answers `404` for a contact outside your workspace.
</Update>

<Update label="September 21, 2026" tags={["API", "Opportunities"]}>
## Opportunities have a source

Where a deal came from is now a field of its own, accepted by `POST /opportunities` and `PATCH /opportunities/{id}` and returned on every opportunity response.

- **`source`** - free text up to 100 characters ("Website form", "Autumn campaign", "Referral").
- **Leave it out when you create a deal** and the linked contact's own source is used, so a deal made from a form keeps the attribution the contact already carries. Send an empty string to clear it.
- It is also a merge tag in messages and automations: `{{deal.source}}`.
</Update>

<Update label="September 21, 2026" tags={["API", "Contacts"]}>
## Contacts have an address, a date of birth and a website

Seven new fields on every contact, accepted by `POST /contacts`, `PATCH /contacts/{id}` and `POST /contacts/bulk`, and returned on every contact response (`null` when not set).

- **`address1`, `city`, `state`, `postal_code`** - text. `postal_code` is always a string, so leading zeros and letters survive (`"02134"`, `"SW1A 1AA"`).
- **`country`** - send an ISO 3166-1 alpha-2 code (`"GR"`) or an English country name (`"Greece"`); it is stored and returned as the code.
- **`date_of_birth`** - a date with no time, `YYYY-MM-DD` (`DD/MM/YYYY` is also accepted). It must be a real past date after 1900.
- **`website`** - a bare domain gets `https://` added.
- **An unknown country or an impossible date is refused** with `422 VALIDATION_ERROR`, and the message names the field and the value. Send an empty string to clear a field.
- **The new fields are also merge tags** in messages and automations: `{{contact.address1}}`, `{{contact.city}}`, `{{contact.country}}`, `{{contact.date_of_birth}}`, `{{contact.full_address}}` and the rest.
</Update>

<Update label="September 17, 2026" tags={["API", "Improvement"]}>
...
- **If your integration branches on `502`**, treat `422` with one of the codes above as a refusal to fix, not an outage. See [Errors](/guides/errors).
</Update>

<Update label="September 16, 2026" tags={["API", "Billing"]}>
...
- **What you pay** - each connected social account is billed from your Developer wallet: $6 each for the first 10, $3 for accounts 11 to 100, $1 above that. Marginal, so 15 accounts held for a full month is $75. See [Billing & Credits](/guides/billing-and-credits).
...
<Update label="September 16, 2026" tags={["Social", "Improvement"]}>
...
- **Prohibited content is refused at creation** with `422 CONTENT_POLICY_VIOLATION`, before anything is scheduled or published. The response links the [Content Policy](https://smartlyq.com/legal/content-policy) and the error is not retryable: edit the post, or contact support if you think the decision is wrong.
...
- **New error code `CONTENT_POLICY_VIOLATION`** - HTTP `422`, not retryable. See [Errors](/guides/errors).
</Update>
