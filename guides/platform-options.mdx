...
| Key | Effect |
|---|---|
| `type` | `post` (timeline, including video) or `reel`. Omit it for a normal post - text, image and video all publish to the timeline without it. `feed`, `video`, `photo`, `image`, `text` are accepted as aliases of `post`, and `reels` as an alias of `reel`. Anything else is rejected with `POST_TYPE_UNSUPPORTED` and a list of the accepted values. |
...
Facebook **stories are not supported**. `type: "story"` is refused rather than
published, because there is no story pipeline behind it - a story request used
to be accepted and then published to the timeline instead.
