...
### X (Twitter)
`content` override and `media_urls` per platform. `first_comments` publish as a **reply thread**: the first replies to the post, each subsequent one replies to the previous (a chained thread). Each comment may be a plain string, or an object `{ "text": "...", "media_urls": ["https://..."] }` to attach up to 4 media items to that reply (X allows media on replies). Example:
