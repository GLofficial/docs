...
For **Facebook, Threads, TikTok, and Reddit**, you can start the connection from your own app without sending the user through the SmartlyQ dashboard.
...
When the user approves, the platform's OAuth callback completes the connection on SmartlyQ's side and — if you supplied `return_url` — redirects the user back to your app. The new account now shows up in `GET /v1/social/accounts`.
</Step>
</Steps>

<Warning>
Other platforms (Instagram, LinkedIn, YouTube, etc.) are connected from the dashboard only. Calling `POST /v1/social/connect/{platform}` with an unsupported platform returns **422 `UNSUPPORTED_PLATFORM`** (the response lists the supported ones). A platform whose OAuth app isn't configured returns **422 `PROVIDER_NOT_CONFIGURED`**.
</Warning>
