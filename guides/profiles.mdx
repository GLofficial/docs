...
Everything stays yours where it matters: billing, rate limits, and usage logs remain on **your** account and wallet - the profile is only a data boundary. A call made on a profile's behalf counts against your workspace's request allowance, not a separate one per profile. Delegation requires the `profiles:manage` scope on the key; profile lifecycle uses `profiles:read` / `profiles:write`.
...
- You can create up to **200 sub-accounts** (profiles). Every profile you still hold counts toward that, whether it is active or paused - only deleting one frees a slot. Trying to create the 201st returns `402 LIMIT_REACHED`; delete any you no longer need, or email support@smartlyq.com to have the limit raised on your account.
- **How many accounts you can connect is set by your wallet balance**, at the rates in [Billing & Credits](/guides/billing-and-credits): enough to cover a month at those rates is enough to connect. $60 funds 10 accounts, $150 funds 40, $430 funds 200. The first 2 are free so you can try the API before funding anything, and auto-recharge counts toward the figure. Adding credit raises the limit immediately - there is nothing to apply for.
