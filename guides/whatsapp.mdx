...
A `422` with code `PLATFORM_UPSTREAM_ERROR` means WhatsApp rejected it, and `error.message` carries its reason - most commonly sending a session message outside the 24-hour window (send a template instead).
...
- **24-hour customer service window** — free-form `text` and `media` messages are only deliverable within 24 hours of the customer's last message to you. Outside it, you must use an approved `template`. A `422` refusal on a session message almost always means the window has closed.
