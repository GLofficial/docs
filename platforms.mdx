...
| <Icon icon="discord" iconType="brands" /> **Discord** | <Icon icon="check" color="#16a34a" /> | <Icon icon="minus" color="#cbd5e1" /> | <Icon icon="minus" color="#cbd5e1" /> | <Icon icon="minus" color="#cbd5e1" /> | <Icon icon="minus" color="#cbd5e1" /> ³ |
...
³ Telegram (bot token) and Discord (server webhook) are connected once from the dashboard's Social Accounts page - no OAuth flow, so headless connect doesn't apply.
</Info>

- **Headless connect** lets you connect a *new* account from your own UI via `POST /v1/social/connect/{platform}` — supported on **all OAuth platforms above** (everything except Telegram and Discord). Poll `GET /v1/social/connect/{state_token}` to learn when the user finished. You can still connect from the [Developer Dashboard](https://app.smartlyq.com/my/developer). See [Connecting accounts](/guides/connecting-accounts).
